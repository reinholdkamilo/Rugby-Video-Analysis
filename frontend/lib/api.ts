...
export type Match = { id: number; organisation_id: number; home_team_id: number; away_team_id: number; match_date: string; sport_type: SportType; competition: string | null; venue: string | null; created_at: string };
export type VideoAsset = { id: number; match_id: number; sport_type: SportType; original_filename: string; content_type: string | null; size_bytes: number; created_at: string };
...
export type PipelineStage = { key: string; label: string; status: "pending" | "running" | "done" | "blocked" | "failed"; progress_percent: number; detail: string; count: number | null };
export type PipelineStatus = { match_id: number; video_asset_id: number | null; status: "pending" | "running" | "done" | "blocked" | "failed"; progress_percent: number; message: string; active_job_id: number | null; stages: PipelineStage[] };
...
export type TimelineEvent = { id: number; match_id: number; video_asset_id: number; event_type: EventType; team: EventTeam; start_seconds: number; end_seconds: number; player_name: string | null; outcome: string | null; notes: string | null; phase_number: number | null; field_zone: string | null; clip_requested: boolean; event_source?: string; trust_status?: string; linked_event_id?: number | null; linked_reason?: string | null; confidence?: number | null; inference_rule?: string | null; created_from_event_ids?: string | null; created_at: string; updated_at: string; clip: EventClip | null };
...
export const api = {
  ...
  matches: { list: (organisationId?: number) => request<Match[]>(`/api/matches${organisationId ? `?organisation_id=${organisationId}` : ""}`), create: (payload: { organisation_id: number; home_team_id: number; away_team_id: number; match_date: string; sport_type?: SportType; competition?: string; venue?: string }) => request<Match>("/api/matches", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(payload) }), delete: (matchId: number) => request<void>(`/api/matches/${matchId}`, { method: "DELETE" }), videos: (matchId: number) => request<VideoAsset[]>(`/api/matches/${matchId}/videos`) },
  pipeline: { status: (matchId: number, videoAssetId?: number) => request<PipelineStatus>(`/api/matches/${matchId}/pipeline${videoAssetId ? `?video_asset_id=${videoAssetId}` : ""}`), run: (matchId: number, videoAssetId?: number) => request<PipelineStatus>(`/api/matches/${matchId}/pipeline/run`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ video_asset_id: videoAssetId ?? null, run_vision: true, run_suggestions: true }) }) },
