...
export async function uploadVideoInChunks(
  ...
  let session = await request<UploadSession>("/api/uploads", {
    method: "POST",
  ...
  for (let index = 0; index < session.total_chunks; index += 1) {
    ...
    session = await request<UploadSession>(`/api/uploads/${session.upload_id}/chunks/${index}`, {
      method: "PUT",
    ...
    onProgress(percent, `Uploading chunk ${session.uploaded_chunks.length} of ${session.total_chunks}`);
  }

  onProgress(97, "Finalising video upload");
  session = await request<UploadSession>(`/api/uploads/${session.upload_id}/complete`, {
    method: "POST",
    signal,
  });
